...
describe Puppet::Face[:azure_servicebus, :current] do
  ...
  before :each do
    ...
    service_bus.any_instance.stubs(:create_topic).with(any_parameters)
  end

  describe 'option validation' do
    describe 'valid options' do
      it 'should not raise any exception' do
        expect { subject.create_topic(@options) }.to_not raise_error
      end
    end
    ...
    describe '(topic_name)' do
      it 'should validate topic name' do
        ...
        expect { subject.create_topic(@options) }.to raise_error(
          ArgumentError,
          /required: topic_name/
        )
      end
