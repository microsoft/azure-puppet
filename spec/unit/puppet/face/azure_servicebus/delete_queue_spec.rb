...
describe Puppet::Face[:azure_servicebus, :current] do
  ...
  before :each do
    ...
    service_bus.any_instance.stubs(:delete_queue).with(any_parameters)
  end

  describe 'option validation' do
    describe 'valid options' do
      it 'should not raise any exception' do
        expect { subject.delete_queue(@options) }.to_not raise_error
      end
    end
    ...
    describe '(queue_name)' do
      it 'should validate queue name' do
        ...
        expect { subject.delete_queue(@options) }.to raise_error(
          ArgumentError,
          /required: queue_name/
        )
      end
