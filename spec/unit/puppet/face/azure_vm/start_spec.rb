...
describe Puppet::Face[:azure_vm, :current] do
  ...
  before :each do
    ...
    vms.any_instance.stubs(:start_virtual_machine).with(anything, anything)
  end

  describe 'option validation' do
    describe 'valid options' do
      it 'should not raise any exception' do
        expect { subject.start(@options) }.to_not raise_error
      end
    end
    ...
    describe '(vm_name)' do
      it 'should validate the vm name' do
        ...
        expect { subject.start(@options) }.to raise_error(
          ArgumentError,
          /required: vm_name/
        )
      end
    ...
    it_behaves_like 'validate authentication credential', :start
